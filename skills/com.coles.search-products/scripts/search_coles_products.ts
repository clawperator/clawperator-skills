...
if (!deviceId) {
  console.error("Usage: npx tsx search_coles_products.ts <device_id> <query> [receiver_package]");
  process.exit(1);
}
...
try {
  ...
  if (snapText) {
    ...
    lines.forEach(line => {
       if (line.includes("text=\"$") || line.includes("Coca-Cola") || line.includes("Coke")) {
