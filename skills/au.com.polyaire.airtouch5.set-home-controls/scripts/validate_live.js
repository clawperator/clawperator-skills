...
function sanitizePreview(value, maxLength = 1000) {
  const text = String(value || "")
    ...
    .replace(/(["'](?:device|deviceId|device_id|serial)["']\s*:\s*["'])[^"']+(["'])/gi, "$1<device_serial>$2")
    .replace(/(["']devices["']\s*:\s*)\[[^\]]*\]/gi, "$1[\"<device_serial>\"]")
    .replace(/\bDevice\s+[A-Za-z0-9._:-]+\b/g, "Device <device_serial>")
    .replace(/\b[A-Z0-9]{6,}\b/g, "<device_serial>")
